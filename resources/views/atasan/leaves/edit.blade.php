                        ...
                        <div class="border-t border-gray-200 dark:border-gray-600">
                            <dl>
                                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">{{ $leave->user->name }}</dd>
                                </div>
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
                                        {{ \Carbon\Carbon::parse($leave->start_date)->format('d F Y') }} s/d {{ \Carbon\Carbon::parse($leave->end_date)->format('d F Y') }}
                                    </dd>
                                </div>
                                <div class="bg-white dark:bg-gray-800 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Dokumen Bukti</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
                                        @if($leave->proof_document)
                                            <a href="{{ asset('storage/' . $leave->proof_document) }}" target="_blank" class="text-blue-600 hover:underline dark:text-blue-400">Lihat Dokumen</a>
                                        @else
                                            Tidak ada
                                        @endif
                                    </dd>
                    ...
                    <form method="POST" action="{{ route('atasan.leaves.update', $leave) }}">
                        ...
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                required>
                                <option value="approved" @selected(old('status', $leave->status) == 'approved')>Setujui</option>
                                <option value="rejected" @selected(old('status', $leave->status) == 'rejected')>Tolak</option>
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="approver_notes" name="approver_notes" rows="3"
                                class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm"
                            >{{ old('approver_notes', $leave->approver_notes) }}</textarea>
